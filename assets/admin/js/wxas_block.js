/**
 * WebXapp plugins Gutenberg integration
 * version 1.0.0
 */
( function ( blocks, element ) {
    ...
    function registerAllPluginBlocks() {
        var wxPluginsData = window['wxas_gb'];
        if ( !wxPluginsData ) {
            return;
        }

        for ( var pluginId in wxPluginsData ) {
            if ( !wxPluginsData.hasOwnProperty( pluginId ) ) {
                continue;
            }

            if ( !wxPluginsData[pluginId].inited ) {
                wxPluginsData[pluginId].inited = true;
                registerPluginBlock( blocks, element, pluginId, wxPluginsData[pluginId] );
            }
        }
    }
        ...
        blocks.registerBlockType( pluginId, {
            ...
            edit: function ( props ) {
                ...
                function showPopup( shortcode, shortcode_id ) {
                    ...
                    var elem = el( 'form', { className: 'wx-container' }, el( 'div', { className: 'wx-container-wrap' + (pluginData.containerClass ? ' ' + pluginData.containerClass : '') }, el( 'span', {
                ...
                function showShortcodeList( shortcode ) {
                    ...
                    shortcodeList.inputs.forEach( function ( inputItem ) {
                        if ( inputItem.type === 'select' ) {
                            children.push( el( 'option', { value: '', dataId: 0 }, wxa_obj.empty_item ) );
